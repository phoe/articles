...
Trying to use `BRANCH-IF`, `BRANCH-WHEN`, or `BRANCH-UNLESS` outside the lexical environment established by `WITH-BRANCHING` will signal a `PROGRAM-ERROR`.

Trying to use a branch name `BRANCH-IF`, `BRANCH-WHEN`, or `BRANCH-UNLESS` that wasn't declared in `WITH-BRANCHING` will signal a `PROGRAM-ERROR`.

--------------

Grab the code from [GitHub](https://github.com/phoe/with-branching).
